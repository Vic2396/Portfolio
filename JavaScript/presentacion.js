...
document.addEventListener("DOMContentLoaded", function() {
    // Simula un retardo en la carga para el ejemplo
    setTimeout(function() {
        document.body.classList.add('carga');
        document.getElementById('precarga').style.display = 'none';
    }, 700); // Ajusta el tiempo de retardo según tus necesidades
});
...
//Mostrar u ocultar texto informativo detras de la imagen
function mostrarTexto(event){
    var contenido = event.currentTarget;
    var allcontenido = document.querySelectorAll('.img, .contenido-3');

    allcontenido.forEach(function(item){
        if (item !== contenido){
            item.classList.remove('mostrarTexto');
            item.querySelector('.texto').scrollTop = 0;
        }
    });
    
    var detalles = contenido.querySelector('.texto');
    if (contenido.classList.contains('mostrarTexto')) {
        contenido.classList.remove('mostrarTexto');
        detalles.scrollTop = 0; // Resetea el scroll del texto oculto
    } else {
        contenido.classList.add('mostrarTexto');
    }
}
